...
def main():
    ...
    for cur_r in missing_rate_list:
        ...
        if dataset_name == 'sims':
            for _, cur_seed  in enumerate([1113]):
                best_ckpt = os.path.join(f'/root/LNLN-main/ckpt/{dataset_name}/best_{key_eval}_{cur_seed}.pth')
            ...
            if key_eval == 'Mult_acc_2':
                ...
                result_avg1 += MAE_avg
                print(f'key_eval: {key_eval}, missing rate: {cur_r}, MAE_avg: {MAE_avg}')
            elif key_eval == 'Corr':
                Corr_avg = test_results_list[0]['Corr'] 
                result_avg1 += Corr_avg
                print(f'key_eval: {key_eval}, missing rate: {cur_r}, Corr_avg: {Corr_avg}')
            ...
            if key_eval == 'Has0_acc_2':
                ...
                #Corr_avg = test_results_list[0]['Corr'] 
                print(
                    f'key_eval: {key_eval}, missing rate: {cur_r}, MAE_avg: {round(MAE_avg, 4)}')
                result_avg1 += MAE_avg
                #result_arg2 += Corr_avg
            elif key_eval == 'Corr':
                Corr_avg = test_results_list[0]['Corr'] 
                print(f'key_eval: {key_eval}, missing rate: {cur_r}, Corr_avg: {round(Corr_avg, 4)}')
                result_avg1 += Corr_avg
